...
const SoilMoistureSensor: React.FC<SoilMoistureSensorProps> = ({ onDataUpdate }) => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setSensorData(prev => {
                const moistureChange = (Math.random() - 0.5) * 8;
                const newValue = Math.max(0, Math.min(100, prev.value + moistureChange));

                let newStatus: 'dry' | 'optimal' | 'wet';
                if (newValue < calibrationData.dryValue) {
                    newStatus = 'dry';
                } else if (newValue > calibrationData.wetValue) {
                    newStatus = 'wet';
                } else {
                    newStatus = 'optimal';
                }
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="mb-6">
                ...
                <div className="w-full bg-gray-200 rounded-full h-4 relative">
                    <div
                        className={`h-4 rounded-full transition-all duration-1000 ${sensorData.status === 'dry' ? 'bg-red-500' :
            ...
            <div className="border-t pt-6">
                ...
                <div className="mt-4 flex space-x-2">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isCalibrating
