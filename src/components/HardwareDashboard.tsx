...
const HardwareDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setHardwareStatus((prev) => {
        ...
        newStatus.soilMoisture.value = Math.max(
          0,
          Math.min(100, prev.soilMoisture.value + moistureChange)
        );
        ...
        // Update LCD display
        newStatus.lcd.line1 = `Soil: ${Math.round(
          newStatus.soilMoisture.value
        )}%`;
        newStatus.lcd.line2 = `Pump: ${prev.pump.running ? "ON" : "OFF"}`;

        // Auto pump control
        if (
          pumpSettings.autoMode &&
          !prev.pump.running &&
          newStatus.soilMoisture.value < pumpSettings.moistureThreshold
        ) {
  ...
  const togglePump = () => {
    setHardwareStatus((prev) => ({
      ...
      pump: {
        ...prev.pump,
        running: !prev.pump.running,
        lastRun: !prev.pump.running ? new Date() : prev.pump.lastRun,
      },
    }));
  };
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div
              className={`px-2 py-1 rounded-full text-xs font-medium ${
                hardwareStatus.esp8266.connected
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-6 rounded-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <div
              className={`px-2 py-1 rounded-full text-xs font-medium flex items-center ${getStatusColor(
                hardwareStatus.soilMoisture.status
              )}`}
            >
          ...
          <div className="space-y-2">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all duration-500 ${
                  hardwareStatus.soilMoisture.status === "dry"
                    ? "bg-red-500"
                    : hardwareStatus.soilMoisture.status === "optimal"
                    ? "bg-green-500"
                    : "bg-blue-500"
                }`}
            ...
            <div className="text-xs text-gray-500">
              Last reading:{" "}
              {hardwareStatus.soilMoisture.lastReading.toLocaleTimeString()}
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-red-50 p-6 rounded-lg">
          ...
          <div className="space-y-3">
            ...
            <div className="text-sm text-gray-600 space-y-1">
              <div>
                Total runtime:{" "}
                {Math.floor(hardwareStatus.pump.totalRuntime / 60)}m{" "}
                {hardwareStatus.pump.totalRuntime % 60}s
              </div>
              {hardwareStatus.pump.lastRun && (
                <div>
                  Last run: {hardwareStatus.pump.lastRun.toLocaleTimeString()}
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setPumpSettings((prev) => ({
                  ...prev,
                  pumpDuration: parseInt(e.target.value),
                }))
              }
              className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-green-500"
            />
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setPumpSettings((prev) => ({
                  ...prev,
                  maxDailyRuns: parseInt(e.target.value),
                }))
              }
              className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-green-500"
            />
